...
本プロジェクトは、実際の課題 ― 航空券予約サービス ― を題材とし、Web開発スキルを向上させることを主な目的として構築しました。

具体的には：
- Webプログラミングのプロトコルの実装演習：HTTPやREST APIの仕組みを理解し、クライアントとサーバ間のリクエスト／レスポンス処理を習得する。
- Spring Bootフレームワークの学習：バックエンド開発、データ管理、権限管理、API構築への応用。
- フロントエンドとバックエンドの連携：ユーザーインターフェース（HTML、CSS、JavaScript）とサーバ（Spring Boot + SQL）の接続を実践的に習得。
- チーム開発・プロジェクト管理スキルの向上：GitHubを活用したソースコード管理、タスク分担、コード統合。
- 学んだ知識の実践的応用：データベース設計、業務ロジックの処理、直感的な航空券予約画面の構築。
